...
- Lowercase only when *not* the first or last word: articles (`a`, `an`, `the`), coordinating conjunctions (`and`, `but`, `or`, `nor`, `for`, `so`/`yet` *as conjunctions*), and prepositions regardless of length (`of`, `to`, `in`, `from`, `with`, `upon`, `into`, etc.).
